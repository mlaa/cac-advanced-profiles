<a id="m"></a>

<?php if ( ! bp_is_user_profile() || bp_is_user_change_avatar() || ( ! empty( $_GET['commons-profile'] ) && 1 === (int) $_GET['commons-profile'] ) ) : ?>
	<?php bp_locate_template( 'cacap/commons-profile.php', true ) ?>
<?php else : ?>

	<?php do_action( 'cacap_body' ); ?>

	<div id="cacap-user-widgets">
		<?php foreach ( cacap_user_widget_instances() as $widget_instance ) : ?>

			<?php // look for visibility in the ['value'] if you don't see it in the instance. 
			if ( ! $widget_instance->visibility && $widget_instance->value['visibility'] ) { 
					$widget_instance->visibility = $widget_instance->value['visibility']; 
			}  
			if ( cacap_user_can_view_widget( $widget_instance->visibility ) ): ?> 
				<div class="cacap-row cacap-widget cacap-widget-<?php echo esc_attr( $widget_instance->widget_type->slug ) ?>">
					<div class="cacap-widget-title"><?php echo $widget_instance->display_title() ?></div>
					<div class="cacap-widget-content"><?php echo $widget_instance->display_content() ?></div>
				</div>
			<?php endif; ?> 
		<?php endforeach; ?>
		</ul>
	</div>
<?php endif ?>
